...
# 当用到sub_test时，sub_test为True，否则为False(使用处理过的sub_test数据集)
def eval_model(model: CTCP, eval: Data, device: torch.device, param: Dict, metric: Metric,
               loss_criteria: _Loss, move_final: bool = False, sub_test:bool = False) -> Dict:
    model.eval()
    model.reset_state()
    metric.fresh()
    epoch_metric = {}
...
def train_model(num: int, dataset: Data, model: CTCP, logger: logging.Logger, early_stopper: EarlyStopMonitor,
    ...
    for epoch in range(param['epoch']):
        ...
        train_loss = []
        # batch_size = 50,x是size为6的tuple,每个元素的size是50，label是size为50
